...
Created by Abdul Moiz Hussain (23K-0553) & Muhammad Hassan Khan (23K-0565).
...
- **Scikit-learn**: Machine learning utilities (TF-IDF, Train/Test Split, Metrics).
- **TensorFlow**: Deep learning framework for the NCF model.
...
6.  **Hybrid Fusion**:
    - **Goal**: Combine scores to balance _personalization_ (NCF) with _relevance_ (CBF).
...
- **Negative (0)**: User has _not_ interacted with the book (Assumed negative).
...
- **Solution**: We use **Content-Based Filtering** to bridge this gap. A new book has no ratings, but it _does_ have a title and tags, allowing us to recommend it immediately if it matches a user's known interests.
...
- **IDF**: Inverse frequency across _all_ books (e.g., "Magic" is rare, so it has high weight; "Book" is common, so it has low weight).
